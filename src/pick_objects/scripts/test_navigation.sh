...
xterm +hold -e "roslaunch turtlebot_gazebo turtlebot_world.launch world_file:=${PICK_OBJECTS_WORLD}" &
sleep 2

MAP_FILE=${PICK_OBJECTS}/maps/training_world.yaml
xterm +hold -e "roslaunch turtlebot_gazebo amcl_demo.launch map_file:=${MAP_FILE} initial_pose_a:=-1.5708" &
sleep 2
